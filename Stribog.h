...
struct stribog_data {
	...
	uint8_t v_0[BLOCK_SIZE];
	uint8_t v_512[BLOCK_SIZE];
	size_t buf_size;
};
...
public:
	...
	Stribog();
	~Stribog();
	void xor_vect(const uint8_t *a, const uint8_t *b, uint8_t *c);
	void add_512(const uint8_t *a, const uint8_t *b, uint8_t *c);
	void sub_s(uint8_t *vect);
	void perm_p(uint8_t *vect);
	void linr_l(uint8_t *vect);
	//void stribog_hash(uint8_t *data, size_t data_len, uint8_t *hash);
	void stribog_hash(uint8_t *hash, const uint8_t *data, size_t data_len);
	void get_key(uint8_t *K, int i);
	void spl(uint8_t *vect);
	void e(uint8_t *K, const uint8_t *m, uint8_t *vect);
	void g(uint8_t *h, uint8_t *N, const uint8_t *m);
	void padding(stribog_data *sdt);
	void init(stribog_data *sdt);
	void step_2(stribog_data *sdt, uint8_t *data);
	void step_3(stribog_data *sdt);
	void hash_update(stribog_data *sdt, uint8_t *data, size_t data_len);
	void get_hash(stribog_data *sdt);
	void hash_reverce(uint8_t *hash);
